...
def class_text_to_int(row_label):
    if row_label == 'text':
        ...
        return 2
    elif row_label == 'connection':
        return 3
    elif row_label == 'data':
        return 4
    elif row_label == 'decision':
        return 5
    elif row_label == 'process':
        return 6
    elif row_label == 'terminator':
        return 7
    else:
        None
...
def create_tf_example(group, path):
    with tf.gfile.GFile(os.path.join(path, '{}'.format(group.file_name)), 'rb') as fid:
    ...
    tf_example = tf.train.Example(features=tf.train.Features(feature={
        ...
        'image/object/name': dataset_util.int64_list_feature(classes),
    }))
    return tf_example
